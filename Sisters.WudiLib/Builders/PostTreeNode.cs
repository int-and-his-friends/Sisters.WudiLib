...
internal class PostTreeNode
{
    ...
    public PostTreeNode(Type type, bool isSpecified = true)
    {
        if (!typeof(Post).IsAssignableFrom(type))
        {
            throw new ArgumentException(
                $"传入的类型必须是 {typeof(Post).FullName} 的子类。",
                nameof(type)
            );
        }
        ...
        foreach (var fieldName in definedKeys)
        {
            ...
            {
                throw new WudiLibBuilderException(
                    $"在上报类型 {type.FullName} 中重复出现了字段 {fieldName} 的约束。"
                );
            }
        }
    ...
    private void AddNodeToThis(PostTreeNode node)
    {
        ...
        {
            var existingNoMoreGeneric = IsNoMoreGeneric(
                llNode.Value._definedKeys,
                node._definedKeys
            );
            if (existingNoMoreGeneric == true)
            {
                ...
                if (node._definedKeys.SetEquals(llNode.Value._definedKeys))
                {
                    ...
                    {
                        throw new WudiLibBuilderException(
                            $"Types {node._type.FullName} and {llNode.Value._type.FullName} have identical field constraint."
                        );
                    }
                }
                continue;
            ...
            {
                throw new WudiLibBuilderException(
                    $"Types {node._type.FullName} and {llNode.Value._type.FullName} have conflict."
                );
            }
    ...
    public void AddType(Type type)
    {
        ...
        for (Type? t = type; t != _type; t = t.BaseType)
        {
            if (t is null)
            {
                throw new InvalidOperationException("添加类型时遇到 null 类型。");
            }
            chain.Push(t);
        }
    ...
    public static bool? IsNoMoreGeneric(ISet<string> compared, ISet<string> comparing)
    {
